...
class MyTest(unittest.TestCase):
    ...
    def test_update(self):
        ...
        core.updtarget("nonupdated", action=lambda s: foo(x),
                       need_if=lambda s: False)
        core.updtarget("updated", action=lambda s: foo(y),
                       need_if=lambda s: True)

        core.do("nonupdated", action="action_if_need")
        core.do("updated", action="action_if_need")

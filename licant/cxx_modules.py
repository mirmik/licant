...
def concat_add_srcdir(base, local, solver, tbase, tlocal):
	if local is None:
		...
		local = list(map(lambda p: os.path.join(
			tlocal.opts["__dir__"], srcdir, os.path.expanduser(p)), local))
	#print(base + local)
	return base + local
		...
		for k, v in self.opts.items():
			if not k in licant.modules.special:
				...
				if v.__class__.__name__ != self.table[k].type:
					print("Option should be object of {}: {}".format(
...
def link_objects(srcs, objs, deps, cxxopts, adddeps):
	...
	for s, o, d in zip(srcs, objs, deps):
		...
		licant.cxx_make.depend(src=s, tgt=d, opts=cxxopts, deps=[
							   s] + adddeps + headers, force=force)
		licant.cxx_make.object(src=s, tgt=o, opts=cxxopts, deps=[
							   s, d] + adddeps + headers)
